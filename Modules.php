...
if(strpos($_REQUEST['modname'],'miscellaneous/')===false)
	...
	echo "<BODY onload='newLoad()' style='overflow-x:hidden; min-winzzdth:900px;'>";
}		
      ...
      <td ><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">
          <tr>
            <td class=\"banner\" valign=\"top\"><table width='100%' border='0' cellspacing='0' cellpadding='0'>
                <tr>
                  <td class=\"school_name\"><h1>UNIQ AMASS</h1></td>
                  <td align=\"left\">&nbsp;</td>
...
echo '<tr><td style="color:#fff;">Subject</td><td></td>
          <td style="color:#fff">Class</td><td></td>
		  <td style="color:#fff">Class Period</td></tr>';
if(isset($_REQUEST['cp_id_miss_attn']))
{
  $get_course=DBGet(DBQuery('SELECT * FROM course_periods WHERE COURSE_PERIOD_ID='.$_REQUEST['cp_id_miss_attn']));
  $_SESSION['UserCourse']=$get_course[1]['COURSE_ID'];
  $get_course=DBGet(DBQuery('SELECT * FROM courses WHERE COURSE_ID='.$get_course[1]['COURSE_ID']));
  $_SESSION['UserSubject']=$get_course[1]['SUBJECT_ID'];
}
...
echo "<SELECT name=subject onChange='this.form.submit();' style='width:100;'>";
if(count($RET)>0)
{
...
echo "<SELECT name=course onChange='this.form.submit();' style='width:100;'>";
if(count($RET)>0)
{
...
{
  $_SESSION['CpvId']=$_REQUEST['cpv_id_miss_attn'];
}
...
foreach($RET as $period)
{
  ...
  $all_cpv_ids=DBGet(DBQuery('SELECT GROUP_CONCAT(ID) as ID FROM course_period_var WHERE COURSE_PERIOD_ID='.$period[COURSE_PERIOD_ID]));
  
     echo "<OPTION id=$period[COURSE_PERIOD_ID] value=$period[ID]".((CpvId()==$period['ID'] || ($all_cpv_ids[1]['ID']!='' && strpos($all_cpv_ids[1]['ID'],CpvId())!='') )?' SELECTED':'').">".$period['TITLE']." - ".$period_det['PERIOD_NAME']." - ".$period_det['DAYS']."</OPTION>";
...
<table cellspacing=0 celpadding=0 border=0 width=100%><tr><td width=60% valign=middle class='welcome'><b>Welcome to Uniq Amass Student Information System</b></td></tr></table> 
...
echo "				<div class=\"page_heading_breadcrumb\"><label id='header' name='header'>showhel</label>&nbsp;";

echo '<div id="showhelp" style="padding-top:10px;"><a href="javascript:void(0);" onclick="inter=setInterval(\'ShowBox(helpdiv, 380, 499, 499, 211, showhelp)\',1);return false;"><b>Help</b></a></div></td></tr></table>';
...
echo "<div id='content' name='content' class=\"txt_wrapper\">";
...
echo "</div>";
  ...
  <tr>
    <td align='center' class='copyright'>
       <center>Ahmadiyya Muslim Education, Inc. 
                </center></td>
  </tr>
</table>
			</td>
