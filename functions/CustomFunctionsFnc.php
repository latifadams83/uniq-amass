...
{	
	...
	if(!$_REQUEST['delete_ok'] &&!$_REQUEST['delete_cancel'])
	{
		...
		echo '<table width="100%"><tr><td width="180px" align="right">Student Enrollment Date:</td><td>';
                ...
                echo '<hr>';
                $total_sems=DBGet(DBQuery('SELECT * FROM school_semesters WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\' ORDER BY MARKING_PERIOD_ID'));
                $each_sem_hq=0;
                foreach($total_sems as $ts)
                {
                    $get_qtrs_usem=DBGet(DBQuery('SELECT * FROM school_quarters WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\' AND SEMESTER_ID=\''.$ts['MARKING_PERIOD_ID'].'\' '));
                    if(count($get_qtrs_usem)>0)
                    $each_sem_hq++;
                }
//                echo count($total_sems).'----'.$each_sem_hq;
                    ...
                    foreach($check_ss as $ss_i=>$ss_d)
                    {
                      $sem++;
                       ...
                       if(count($check_sq)>0)
                        ...
                        $t++;
                        echo '<input type=hidden id=round_'.$t.' value='.(count($total_sems)==$each_sem_hq?$j:$j-1).'>'; 
