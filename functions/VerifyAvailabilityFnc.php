...
function VerifyVariableSchedule_Update($columns)
{
    ...
    $sql_same_period="SELECT cp.COURSE_PERIOD_ID FROM course_periods cp,course_period_var cpv  WHERE  days = '".$columns[DAYS]."' AND period_id='".$per_id."' AND ROOM_ID=$columns[ROOM_ID] AND TEACHER_ID=$teacher AND cpv.course_period_id = cp.course_period_id";
    $same_cp_RET=DBGet(DBQuery($sql_same_period));
    if(count($same_cp_RET)>0 && $same_cp_RET[1]['COURSE_PERIOD_ID']==$_REQUEST['cp_id'])
    {
        return 'Same meeting days already exists.';
    }
    else
    {
      $sql_op="SELECT cp.COURSE_PERIOD_ID FROM course_periods  cp LEFT JOIN course_period_var cpv ON (cp.course_period_id=cpv.course_period_id) WHERE cpv.PERIOD_ID='".$per_id."'  AND (secondary_teacher_id IN ($all_teacher) OR teacher_id IN ($all_teacher)){$mp_append_sql}{$days_append_sql}";
