...
function formcheck_Timetable_course_F4()
{
    ...
    frmvalidator.addValidation("tables[course_subjects][new][TITLE]", "maxlen=50", "Max length for subject is 50 characters");
    }
    else
    {
        frmvalidator.addValidation("inputtables[course_subjects][" + subject_id + "][TITLE]", "req", "Please enter the course title ");
        frmvalidator.addValidation("inputtables[course_subjects][" + subject_id + "][TITLE]", "maxlen=50", "Max length for course title is 50 characters");
    }
...
function formcheck_school_grad_config()
{
    ...
    for (i = 0; i < mp_per.length; i++) {
        if(mp_per[i].value!=='')
            frmvalidator.addValidation(mp_per[i].name, "num", "Final grading percentage allows only numeric value");
    }
}
