	...
	foreach($_REQUEST['tables'] as $id=>$columns)
	{
		...
		if($id!='new')
		{
			...
			foreach($columns as $column=>$value)
			{
                                ...
                                if($column=='ASSIGNED_DATE'&& $value!='' && $due_date_sql[1]['ASSIGNED_DATE']!=$value && $table=='gradebook_assignments')
                                {
...
$grade_assign_qr=  DBGet(DBQuery("SELECT COUNT(STUDENT_ID) AS TOT FROM  gradebook_grades WHERE ASSIGNMENT_ID='$id'"));
                        ...
                        {
                            $f=1;
                            ShowErrPhp('Cannot modify the end date or start date because grade has been assigned to the student for this assignment1.');
                        }
                         }
 ...
 $sql='SELECT sum(FINAL_GRADE_PERCENT) AS TOTAL_PERCENT FROM gradebook_assignment_types WHERE assignment_type_id in(select assignment_type_id from gradebook_assignment_types  where COURSE_PERIOD_ID=\''.$course_period_id.'\')';	
	...
	elseif($_REQUEST['assignment_type_id'])
	{
                ...
                if($programconfig['WEIGHT']=='Y')
		{
			...
			$header .= '<TD>' . TextInput($RET['FINAL_GRADE_PERCENT'],'tables['.$_REQUEST['assignment_type_id'].'][FINAL_GRADE_PERCENT]',($RET['FINAL_GRADE_PERCENT']!=0?'':'<FONT color=red>').'Weight Percent'.($RET['FINAL_GRADE_PERCENT']!=0?'':'</FONT>')) . '</TD>';
