...
if($_REQUEST['modname']=='grades/HonorRoll.php')
{
	...
	$LO_columns = array('FULL_NAME'=>'Student','STUDENT_ID'=>'Student ID','ALT_ID'=>'Placed ID','GRADE_ID'=>'Grade','PHONE'=>'Phone');
...
echo '<div style="overflow:auto; width:820px; overflow-x:scroll;">';
                        ...
                        {
                            foreach($students_RET as $key=>$value)
                            {
                                if($value['STUDENT_ID']== UserStudentID())
                                    $onlyStu_list[1]=$value;
                            }
                            ListOutputExcel($onlyStu_list,$columns,$extra['singular'],$extra['plural'],$link,$extra['LO_group'],$extra['options']);
                        }
                        else
