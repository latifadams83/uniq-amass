...
if($_REQUEST['student_id']=='new')
{
   unset($_SESSION['students_order']);
	...
	echo '<DIV id=student_name><div style="font-size:14px; font-weight:bold;" onclick=\'addHTML("<TABLE><TR><TD>'.str_replace('"','\"',TextInput($student['FIRST_NAME'],'students[FIRST_NAME]','<FONT color=red>First</FONT>','maxlength=50 style="font-size:14px; font-weight:bold;"',false)).'</TD><TD>'.str_replace('"','\"',TextInput($student['MIDDLE_NAME'],'students[MIDDLE_NAME]','Middle','size=3 maxlength=50 style="font-size:14px; font-weight:bold;"',false)).'</TD><TD>'.str_replace('"','\"',TextInput($student['LAST_NAME'],'students[LAST_NAME]','<FONT color=red>Last</FONT>','maxlength=50 style="font-size:14px; font-weight:bold;"',false)).'</TD><TD></TD></TR></TABLE>","student_name",true);\'>'.$student['FIRST_NAME'].' '.$student['MIDDLE_NAME'].' '.$student['LAST_NAME'].' '.'</div></DIV>';
echo'</td></tr>';

echo '<tr><td>Estimated Comp. Date </td><td>:</td><td>'.DateInputAY($student['ESTIMATED_GRAD_DATE'],'students[ESTIMATED_GRAD_DATE]','1').'</td></tr>';
...
echo '<tr><td>Placed ID</td><td>:</td><td>';
...
echo NoInput($options[$student['GRADE_ID']]);
else
...
echo TextInputModHidden($student['USERNAME'],'students[USERNAME]','','class=cell_medium onkeyup="usercheck_init_student(this)"');
echo '<span id="ajax_output_st"></span>';
echo'</td></tr>';
echo '<tr><td>Password</td><td>:</td><td>';
echo TextInputModHidden(array($student['PASSWORD'],str_repeat('*',strlen($student['PASSWORD']))),'students[PASSWORD]','','class=cell_medium onkeyup=passwordStrength(this.value)','AUTOCOMPLETE = off');
...
echo CheckboxInput($student['IS_DISABLE'],'students[IS_DISABLE]','','CHECKED',$new,'<IMG SRC=assets/check.gif width=15>','<IMG SRC=assets/x.gif width=15>','','onClick="toggleDisableStu(this);"');
echo '<input type="hidden" name=student_enable value="" id="student_enable"/>';
echo '</td></tr>';
}
}
echo'</table>';
