...
$edit_per_prnt = DBGet(DBQuery('SELECT CAN_EDIT FROM profile_exceptions WHERE MODNAME=\'students/Student.php&category_id=5\' AND PROFILE_ID=3'));
	...
	if($_REQUEST['goal_id']!='new')
	{	
		if(((User('PROFILE') == 'admin') && isset($edit_per_adm[1]['CAN_EDIT'])) || ((User('PROFILE') == 'teacher' && AllowEdit()) && isset($edit_per_teach[1]['CAN_EDIT'])) || ((User('PROFILE') == 'parent') && isset($edit_per_prnt[1]['CAN_EDIT'])))
		{
			$header .= '<TR>';
