...
if(count($enrollment_RET)>0)
    echo "<table width=100%><tr><td width=105>".DrawLogo()."</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">Add / Drop Report</div></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br />Uniq Amass</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";

ListOutputPrint($enrollment_RET,$columns,'Schedule Record','Schedule Records');
}
}
else {
echo "<FORM name=addr id=addr action='ForExport.php?modname=".strip_tags(trim($_REQUEST[modname]))."&head_html=Add+/+Drop+Report&modfunc=save&sday=$start_date&eday=$end_date&include_inactive=".strip_tags(trim($_REQUEST[include_inactive]))."&_openSIS_PDF=true&flag=list' method=POST target=_blank>";
