...
##############################################List Output Generation##################################################
	...
	echo "<FORM id='search' name='search' method=POST action=Modules.php?modname=".strip_tags(trim($_REQUEST[modname])).">";
                ...
                {
                        echo CreateSelect($grade_level_RET, 'id', 'All', 'Select Grade Level: ', 'Modules.php?modname='.strip_tags(trim($_REQUEST['modname'])).'&id=');
                }	
	echo '</td></tr></table>';
	if(clean_param($_REQUEST['id'],PARAM_ALPHANUM))
	{
