...
{	echo '<style type="text/css">.print_wrapper{font-family:arial;font-size:12px;}.print_wrapper table table{border-right:1px solid #666;border-bottom:1px solid #666;}.print_wrapper table td{font-size:12px;}</style>';
	...
	echo "<tr><td  style=\"font-size:15px; font-weight:bold; padding-top:10px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">List of Events</div></td><td align=right style=\"padding-top:10px;\">". ProperDate(DBDate()) ."<br />Uniq Amass</td></tr><tr><td colspan=2 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
	...
	elseif(clean_param($_REQUEST['button'],PARAM_ALPHAMOD)=='Delete')
	{
		if(DeletePrompt('event','delete','y'))
		{
			DBQuery("DELETE FROM calendar_events WHERE ID='".paramlib_validation($column=EVENT_ID,$_REQUEST[event_id])."'");
                        ...
                        echo '<SCRIPT language=javascript>window.close();</script>';
//			
                      echo '<SCRIPT language=javascript>window.location.href = "Modules.php?modname='.$_REQUEST['modname'].'";</script>';
                        unset($_REQUEST['values']);
			unset($_SESSION['_REQUEST_vars']['values']);
	...
	else
	{
		...
		echo '<TR><TD>Notes </TD>'.(User('PROFILE')=='admin'?'<TD>'.TextAreaInput(html_entity_decode(html_entity_decode($RET[1]['DESCRIPTION'])),'values[DESCRIPTION]','','style=width:380px;height:200px;').'</TD>':'<TD style=width:380px; align=left>'.html_entity_decode(html_entity_decode($RET[1]['DESCRIPTION'])).'</TD>').'</TR>';
		if(AllowEdit())
		{
                        if(User('PROFILE')=='admin')
