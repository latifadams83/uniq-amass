...
echo TextInputModHidden($staff['USERNAME'],'login_authentication[USERNAME]','','size=25 maxlength=100 class=cell_floating  onkeyup="usercheck_init(this)"');
echo '<div id="ajax_output"></div>';
echo '</TD></tr><tr><td style=width:100px>Password</td><td>:</td><td>';
//for adding new user
if(!isset($staff['STAFF_ID']))
{
 echo TextInputModHidden(array($staff['PASSWORD'],str_repeat('*',strlen($staff['PASSWORD']))),'login_authentication[PASSWORD]','',"size=25 maxlength=100 class=cell_floating AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);validate_password(this.value);");   
}
//for existing users while updating
 else
{
   echo TextInputModHidden(array($staff['PASSWORD'],str_repeat('*',strlen($staff['PASSWORD']))),'login_authentication[PASSWORD]','',"size=25 maxlength=100 class=cell_floating AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);validate_password(this.value,$staff[STAFF_ID]);"); 
}
