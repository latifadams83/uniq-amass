	...
	if(isset($_REQUEST['school_info_id']))
	{
		...
		if($_REQUEST['school_info_id']!='0' && $_REQUEST['school_info_id']!=='old')
		{
			...
			echo '<FIELDSET><LEGEND><FONT color=gray>Official Information</FONT></LEGEND><TABLE width=100%>';
                        ...
                        echo '<TR><td><span class=red>*</span>Category</td><td>:</td><TD style=\"white-space:nowrap\"><table cellspacing=0 cellpadding=0 cellspacing=0 cellpadding=0 border=0><tr><td>'.$this_school['CATEGORY'].'</td><td>';                           
                        else
			...
			echo '<FIELDSET><LEGEND><FONT color=gray>openSIS Access Information</FONT></LEGEND>';
		        ...
		        echo '<TR><td>Username<span class=red>*</span></td><td>:</td><TD>';
                        if(!$this_school_mod['USERNAME'])
                                ...
                                echo TextInputModHidden('','USERNAME','','size=20 maxlength=50 class=cell_floating onblur="usercheck_init_staff(this)"');
			...
			echo '<TR><td>Password<span class=red>*</span></td><td>:</td><TD>';
                        if(!$this_school_mod['PASSWORD'])
                                {
                                echo TextInputModHidden('','PASSWORD','','size=20 maxlength=100 class=cell_floating AUTOCOMPLETE = off onblur=passwordStrength(this.value);validate_password_staff(this.value);');
                                ...
                                echo TextInputModHidden(array($this_school_mod['PASSWORD'],str_repeat('*',strlen($this_school_mod['PASSWORD']))),'staff_school[PASSWORD]','','size=20 maxlength=100 class=cell_floating AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);validate_password(this.value);');
                                }
